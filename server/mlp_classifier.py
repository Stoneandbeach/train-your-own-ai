...
from server.config import INPUT_SIZE
from server.model_interface import ArchitectureSpec, Classifier, Metrics


def build_mlp(widths: list[int], num_classes: int) -> nn.Sequential:
...
class MLPClassifier(Classifier):
    ...
    def configure(self, spec: ArchitectureSpec) -> None:
        self.spec = spec
        self.model = build_mlp(spec.layers.widths, spec.num_classes)
        self.model.eval()
    ...
    def predict_with_activations(
        ...
        self.model.eval()
        with torch.no_grad():
            x = torch.from_numpy(image).float().unsqueeze(0)  # (1, 28, 28)
            activations: list[np.ndarray] = []
            for layer in self.model:
                x = layer(x)
                if isinstance(layer, nn.ReLU):
                    activations.append(x.squeeze(0).numpy().copy())
            probs = F.softmax(x, dim=1).squeeze(0).numpy()
    ...
    def get_edge_weights(self) -> list[list[list[float]]]:
        ...
        return [lin.weight.detach().numpy().tolist() for lin in linears]
